...
func _enter_tree() -> void:
        ...
        # Load the plugin icon lazily so headless environments without
        # imported SVG support do not crash during script parsing.
        var icon: Texture2D = null
        if ResourceLoader.exists("res://addons/tile_to_gridmap/TileToGridMap.svg"):
                icon = load("res://addons/tile_to_gridmap/TileToGridMap.svg")
        add_custom_type("T2GTerrainLayer", "TileToGrid", preload("res://addons/tile_to_gridmap/t2g_terrain_layer.gd"), icon)
        add_custom_type("T2GProcGenManager", "TileToGrid", preload("res://addons/tile_to_gridmap/t2g_proc_gen_manager.gd"), icon)
        add_custom_type("T2GProps", "TileToGrid", preload("res://addons/tile_to_gridmap/t2g_props.gd"), icon)
