...
if ($godotExe -ieq 'godot4') {
  Get-Content 'logs/ascii_commands.txt' | & $godotExe --headless --path . --script scripts/tools/ascii_console.gd 2>&1 | Tee-Object -FilePath 'logs/ascii_smoke.log' | Out-Null
} else {
  $p2 = Start-Process -FilePath $godotExe -ArgumentList '--headless','--path','.', '--script','scripts/tools/ascii_console.gd' -NoNewWindow -PassThru -RedirectStandardInput 'logs\ascii_commands.txt' -RedirectStandardOutput 'logs\ascii_smoke.log' -RedirectStandardError 'logs\ascii_smoke.err.log'
