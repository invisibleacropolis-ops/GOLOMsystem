...
set +e
"$(dirname "$0")/godot4.sh" --headless --verbose --disable-dotnet --path . --script scripts/test_runner.gd 2>&1 | tee logs/headless_tests.log
...
cat logs/ascii_commands.txt | "$(dirname "$0")/godot4.sh" --headless --verbose --disable-dotnet --path . --script scripts/tools/ascii_console.gd -- --pipe 2>&1 | tee logs/ascii_smoke.log >/dev/null
